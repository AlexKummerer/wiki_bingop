...
def display_bingo_card(card, marked, size=5):
    ...
    print(f"{YELLOW}Bingo Card:{RESET}")
    for i in range(size):
        ...
        for j in range(size):
            ...
            for k in range(len(wrapped_category)):
                row_lines[
                    k] += f"{wrapped_category[k]:<{column_width + 4}} {PURPLE}| {RESET}"

            for k in range(len(wrapped_category), size):
                row_lines[k] += f"{' ' * (column_width + 4)}{PURPLE} | {RESET}"

        for line in row_lines:
            print(line)
        print(f"{PURPLE}-{RESET}" * (column_width * size + 3 * (size - 1)))
